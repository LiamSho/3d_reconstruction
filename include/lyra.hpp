...
struct print {
    print(const char *scope_name = nullptr) : scope(scope_name) {
        if (is_debug)
            print::depth() += 1;
        if (scope)
            debug(scope, "...");
    ...
    template <typename... A> void debug(A... arg) {
        if (is_debug) {
            std::cerr << "[DEBUG]"
...
class args {
  public:
    ...
    args(const It &start, const It &end)
        : m_exeName(*start), m_args(start + 1, end) {
    }
...
template <class F> struct is_invocable {
    template <class U>
    static auto test(U *p)
...
template <typename... Ts> using valid_t = typename make_void<Ts...>::type;
...
template <typename T> bool to_string(const T &source, std::string &target) {
    std::stringstream ss;
    ss << source;
    ss >> target;
    return !ss.fail();
...
template <typename T>
inline bool to_string(LYRA_CONFIG_OPTIONAL_TYPE<T> &source,
                      std::string &target) {
    if (source)
...
template <typename, typename = void> struct validate_from_string {
...
struct validate_from_string<
    T, typename std::enable_if<std::is_unsigned<
           typename detail::remove_cvref<T>::type>::value>::type> {
...
template <typename S> inline bool from_string(S const &source, bool &target) {
    std::string srcLC;
...
class result_base {
  public:
    ...
    bool is_ok() const {
        return kind_ == result_kind::ok;
    }
    std::string message() const {
        return message_;
    }
    [[deprecated]] std::string errorMessage() const {
        return message();
    }

  protected:
    enum class result_kind { ok, error };
    ...
    explicit result_base(const result_base &other)
        : kind_(other.kind_), message_(other.message_) {
    }
...
template <typename T> class result_value_base : public result_base {
  public:
    using value_type = T;
  ...
  protected:
    ...
    explicit result_value_base(result_kind kind, const value_type &val,
                               const std::string &message = "")
        : result_base(kind, message) {
    ...
    explicit result_value_base(const result_base &other) : result_base(other) {
    }
...
template <> class result_value_base<void> : public result_base {
  public:
...
template <typename T> class basic_result : public result_value_base<T> {
  public:
    ...
    static basic_result error(value_type const &val,
...
template <> class basic_result<void> : public result_value_base<void> {
  public:
...
enum class parser_result_type { matched, no_match, short_circuit_all };
...
struct BoundRef : NonCopyable {
    ...
    virtual size_t get_value_count() const {
        return 0;
    }
    virtual std::string get_value(size_t) const {
        return "";
    }
};
...
template <typename L> struct BoundLambda : BoundValueRefBase {
    ...
    auto setValue(std::string const &arg) -> parser_result override {
        return invokeLambda<typename unary_lambda_traits<L>::ArgType>(m_lambda,
                                                                      arg);
    }
};
...
template <typename T> struct BoundVal : BoundValueRef<T> {
    T value;

    BoundVal(T &&v) : BoundValueRef<T>(value), value(v) {
    }

    BoundVal(BoundVal &&other)
        : BoundValueRef<T>(value), value(std::move(other.value)) {
    }
...
*/
template <typename T> struct choices_set : choices_base {
    std::vector<T> values;

    template <typename... Vals>
    explicit choices_set(Vals... vals) : choices_set({vals...}) {
    }

    explicit choices_set(const std::vector<T> &vals) : values(vals) {
    }
  ...
  protected:
    explicit choices_set(std::initializer_list<T> const &vals) : values(vals) {
    }
};

template <> struct choices_set<const char *> : choices_set<std::string> {
    template <typename... Vals>
    explicit choices_set(Vals... vals) : choices_set<std::string>(vals...) {
    }
};
...
*/
template <typename Lambda> struct choices_check : choices_base {
...
enum class token_type { unknown, option, argument };
...
class basic_token_name {
  public:
    ...
    friend string_type operator+(const_pointer lhs,
                                 const basic_token_name &rhs) {
...
struct token {
    ...
    token(token_type t, const token_name &n) : type(t), name(n) {
    }
...
class token_iterator {
  public:
    ...
    explicit token_iterator(Span const &args, const option_style &opt_style)
        : style(opt_style), args_i(args.begin()), args_e(args.end()),
    ...
    bool has_short_option_prefix() const noexcept {
        return (args_i != args_e) && is_prefixed(style.short_option_prefix,
                                                 style.short_option_size,
                                                 *args_i);
    }

    bool has_long_option_prefix() const noexcept {
        return (args_i != args_e) && is_prefixed(style.long_option_prefix,
                                                 style.long_option_size,
                                                 *args_i);
    }
    ...
    token option() const {
        if (has_long_option_prefix()) {
            ...
            return {token_type::option,
                    prefix_value(style.short_option_prefix,
                                 style.short_option_size) +
                        (*args_i)[args_i_sub]};
        }
        return token();
    ...
    token value() const {
        if (has_option_prefix() && has_value_delimiter())
            return token(
                token_type::argument,
                args_i->substr(args_i->find_first_of(style.value_delimiters) +
                               1));
    ...
    static bool is_prefixed(const std::string &prefix, std::size_t size,
                            const std::string &s) {
  ...
  private:
    ...
    inline bool is_prefix_char(const std::string &prefix, std::size_t size,
                               std::string::value_type c) const noexcept {
        return !prefix.empty() && size > 0 &&
               prefix.find(c) != std::string::npos;
    }

    inline std::string prefix_value(const std::string &prefix,
                                    std::size_t size) const {
        return std::string(static_cast<typename std::string::size_type>(size),
                           prefix[0]);
    }
};
...
*/ // end::reference[]
template <typename T> detail::BoundVal<T> val(T &&v) {
...
class parse_state {
  public:
    parse_state(parser_result_type type, token_iterator const &remaining_tokens,
                size_t parsed_tokens = 0)
        : result_type(type), tokens(remaining_tokens) {
        (void)parsed_tokens;
    }

    parser_result_type type() const {
        return result_type;
    }
    token_iterator remainingTokens() const {
        return tokens;
    }
...
struct parser_cardinality {
    ...
    parser_cardinality(size_t a, size_t b) : minimum(a), maximum(b) {
    }
...
class parse_result : public detail::basic_result<detail::parse_state> {
  public:
    ...
    parse_result(const base &other) : base(other) {
    }
};
...
class parser {
  public:
    ...
    [[deprecated]] help_text get_help_text() const {
        return {};
    }
    [[deprecated]] std::string get_usage_text() const {
        return "";
    }
    [[deprecated]] std::string get_description_text() const {
        return "";
    }
    ...
    virtual result validate() const {
        return result::ok();
    }
    virtual std::unique_ptr<parser> clone() const {
        return nullptr;
    }
...
template <typename Derived> class composable_parser : public parser {};
...
class bound_parser : public composable_parser<Derived> {
  protected:
    ...
    bound_parser(std::shared_ptr<detail::BoundRef> const &ref,
  ...
  public:
    ...
    bound_parser(
        ...
        typename std::enable_if<detail::is_invocable<Lambda>::value,
                                ctor_lambda_e>::type = ctor_lambda_e::val);
...
template <typename Derived>
Derived &
bound_parser<Derived>::operator()(std::string const &help_description_text) {
...
template <typename Derived> Derived &bound_parser<Derived>::optional() {
...
template <typename Derived> Derived &bound_parser<Derived>::required(size_t n) {
    if (m_ref->isContainer())
...
template <typename Derived>
template <typename Lambda, typename std::enable_if<
                               detail::is_invocable<Lambda>::value, int>::type>
...
template <typename Derived> std::string bound_parser<Derived>::hint() const {
    return m_hint;
}
...
class exe_name : public composable_parser<exe_name> {
  public:
    exe_name() : m_name(std::make_shared<std::string>("<executable>")) {
    }

    explicit exe_name(std::string &ref);

    template <typename LambdaT> explicit exe_name(LambdaT const &lambda);
...
inline exe_name::exe_name(std::string &ref) : exe_name() {
...
template <typename LambdaT>
exe_name::exe_name(LambdaT const &lambda) : exe_name() {
...
class arguments : public parser {
  public:
    enum evaluation { any = 0, sequence = 1 };

    arguments() = default;

    arguments(evaluation e) : eval_mode(e) {
    }
    ...
    template <typename T> arguments operator|(T const &other) const {
    ...
    virtual std::string
    get_usage_text(const option_style &style) const override {
        std::ostringstream os;
        for (auto const &p : parsers) {
            std::string usage_text = p->get_usage_text(style);
            if (usage_text.size() > 0) {
                if (os.tellp() != std::ostringstream::pos_type(0))
                    os << " ";
    ...
    virtual std::string
    get_description_text(const option_style &style) const override {
        std::ostringstream os;
        for (auto const &p : parsers) {
            if (p->is_group())
                continue;
    ...
    parse_result parse(detail::token_iterator const &tokens,
        ...
        switch (eval_mode) {
        case any:
            return parse_any(tokens, style);
        case sequence:
            return parse_sequence(tokens, style);
        }
...
inline arguments::arguments(const arguments &other)
    : parser(other), opt_style(other.opt_style), eval_mode(other.eval_mode) {
...
template <typename T> T &arguments::get(size_t i) {
...
struct default_parser_customization : parser_customization {
    std::string token_delimiters() const override {
        return " =";
    }
    std::string option_prefix() const override {
        return "-";
    }
};
...
inline group::group() : m_cardinality(0, 1) {
}
...
inline group::group(const group &other)
    : arguments(other), success_signal(other.success_signal),
...
class cli : protected arguments {
  public:
    ...
    template <typename T> cli operator|(T const &other) const;

    struct value_result {
      public:
        explicit value_result(const parser *p) : parser_ref(p) {
        }
    ...
    [[deprecated]] parse_result
    parse(args const &args, const parser_customization &customize) const {
        return this->parse(args,
                           option_style(customize.token_delimiters(),
                                        customize.option_prefix(),
                                        2,
                                        customize.option_prefix(),
                                        1));
    }
...
inline cli::cli(const cli &other)
    : arguments(other), m_exeName(other.m_exeName) {
}
...
template <typename T> inline cli cli::operator|(T const &other) const {
...
inline parse_result cli::parse(args const &args,
                               const option_style &style) const {
...
class literal : public parser {
  public:
    ...
    virtual std::string
    get_description_text(const option_style &) const override {
        return description;
    }
...
inline literal::literal(std::string const &n) : name(n) {
}
...
class command : public group {
  public:
    ...
    command(const std::string &n, const std::function<void(const group &)> &f);

    command &help(const std::string &text);
    command &operator()(std::string const &description);

    template <typename P> command &add_argument(P const &p);
    template <typename P> command &operator|=(P const &p);
...
inline command::command(const std::string &n,
                        const std::function<void(const group &)> &f)
    : group(f) {
    this->sequential()
        .add_argument(literal(n))
        .add_argument(group().required());
...
template <typename P> command &command::add_argument(P const &p) {
    this->get<group>(1).add_argument(p);
    return *this;
}
template <typename P> command &command::operator|=(P const &p) {
    return this->add_argument(p);
}
...
class opt : public bound_parser<opt> {
  public:
    ...
    opt(T &ref, std::string const &hint,
        typename std::enable_if<!detail::is_invocable<T>::value,
                                ctor_ref_e>::type = ctor_ref_e::val);

    template <typename L>
    opt(L const &ref, std::string const &hint,
        typename std::enable_if<detail::is_invocable<L>::value,
                                ctor_lambda_e>::type = ctor_lambda_e::val);
        ...
        if (remainingTokens && remainingTokens.has_option_prefix()) {
            ...
            if (is_match(token.name, style)) {
                if (m_ref->isFlag()) {
                    ...
                    if (!flag_result)
                        return parse_result(flag_result);
                    LYRA_PRINT_DEBUG(
                        "(=)", get_usage_text(style), "==", token.name);
  ...
  protected:
    ...
    bool is_match(std::string const &opt_name,
                  const option_style &style) const {
        auto opt_normalized = normalise_opt(opt_name, style);
        for (auto const &name : opt_names) {
            if (normalise_opt(name, style) == opt_normalized)
                return true;
        }
        return false;
    }

    std::string normalise_opt(std::string const &opt_name,
                              const option_style &style) const {
    ...
    std::string format_opt(std::string const &opt_name,
                           const option_style &style) const {
...
template <typename L>
opt::opt(L const &ref, typename std::enable_if<detail::is_invocable<L>::value,
...
template <typename L>
opt::opt(L const &ref, std::string const &hint,
         typename std::enable_if<detail::is_invocable<L>::value,
                                 opt::ctor_lambda_e>::type)
    : bound_parser(ref, hint) {
}
...
class main final : protected cli {
  ...
  public:
    ...
    template <typename T> main &operator()(const T &parser);
    template <typename T> main &add_argument(const T &parser);
    template <typename T> main &operator|=(const T &parser);

    template <typename V>
    main &operator()(std::initializer_list<std::string> arg_names,
                     V &&default_value);
    template <typename V>
    main &operator()(const std::string &arg_name, V &&default_value);

    template <typename L> int operator()(const args &argv, L action);
    template <typename L> int operator()(int argc, const char **argv, L action);
...
template <typename V>
main &main::operator()(std::initializer_list<std::string> arg_names,
                       V &&default_value) {
...
template <typename L> int main::operator()(const args &argv, L action) {
    auto cli_result = cli::parse(argv);
    if (!cli_result)
        std::cerr << cli_result.message() << "\n\n";
