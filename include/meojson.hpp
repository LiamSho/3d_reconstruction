...
#ifndef INC_3D_RECONSTRUCTION_MEOJSON_HPP
#define INC_3D_RECONSTRUCTION_MEOJSON_HPP
...
class value {
  ...
  public:
    ...
    template <typename Type = value> std::optional<Type> find(size_t pos) const;
  ...
  private:
    ...
    decltype(auto)
    get(std::tuple<KeysThenDefaultValue...> keys_then_default_value,
        std::index_sequence<KeysIndexes...>) const;

    template <typename T, typename FirstKey, typename... RestKeys>
    decltype(auto) get_aux(T &&default_value, FirstKey &&first,
                           RestKeys &&...rest) const;
...
class array {
  ...
  public:
    ...
    template <typename ArrayType,
              typename EnableT = std::enable_if_t<std::is_constructible_v<
                  value, typename ArrayType::value_type>>>
    array(ArrayType arr);
...
class object {
  ...
  public:
    ...
    template <typename MapType,
              typename EnableT = std::enable_if_t<std::is_constructible_v<
                  value_type, typename MapType::value_type>>>
    object(MapType map);
...
template <typename Type> MEOJSON_INLINE bool value::is() const noexcept {
...
template <typename... KeysThenDefaultValue>
MEOJSON_INLINE decltype(auto)
value::get(KeysThenDefaultValue &&...keys_then_default_value) const {
    return get(
...
template <typename... KeysThenDefaultValue, size_t... KeysIndexes>
MEOJSON_INLINE decltype(auto)
value::get(std::tuple<KeysThenDefaultValue...> keys_then_default_value,
           std::index_sequence<KeysIndexes...>) const {
    constexpr unsigned long DefaultValueIndex =
        sizeof...(KeysThenDefaultValue) - 1;
...
template <typename T, typename FirstKey, typename... RestKeys>
MEOJSON_INLINE decltype(auto)
value::get_aux(T &&default_value, FirstKey &&first, RestKeys &&...rest) const {
    if constexpr (std::is_constructible<std::string, FirstKey>::value) {
        return is_object() ? as_object()
                                 .get(std::forward<FirstKey>(first))
                                 .get_aux(std::forward<T>(default_value),
                                          std::forward<RestKeys>(rest)...)
                           : default_value;
    } else if constexpr (std::is_integral<typename std::remove_reference<
                             FirstKey>::type>::value) {
        ...
        static_assert(
            !sizeof(FirstKey),
            "Parameter must be integral or std::string constructable");
    }
}

template <typename T, typename UniqueKey>
MEOJSON_INLINE decltype(auto) value::get_aux(T &&default_value,
                                             UniqueKey &&first) const {
    if constexpr (std::is_constructible<std::string, UniqueKey>::value) {
        return is_object() ? as_object().get(std::forward<UniqueKey>(first),
                                             std::forward<T>(default_value))
                           : default_value;
    } else if constexpr (std::is_integral<typename std::remove_reference<
                             UniqueKey>::type>::value) {
        return is_array() ? as_array().get(std::forward<UniqueKey>(first),
                                           std::forward<T>(default_value))
                          : default_value;
    } else {
        static_assert(
...
template <typename Type> MEOJSON_INLINE Type value::as() const {
...
MEOJSON_INLINE array::array(ArrayType arr) {
    _array_data.assign(std::make_move_iterator(arr.begin()),
...
MEOJSON_INLINE std::optional<Type> array::find(size_t pos) const {
    static_assert(std::is_constructible<Type, value>::value,
                  "Type can NOT be constructed by value");
    if (!contains(pos)) {
        return std::nullopt;
    }
...
object::format(bool ordered, std::string shift_str,
               size_t basic_shift_count) const {
    std::string shift;
    for (size_t i = 0; i != basic_shift_count + 1; ++i) {
        shift += shift_str;
...
MEOJSON_INLINE object object::operator|(object &&rhs) & {
    ...
    temp._object_data.insert(std::make_move_iterator(rhs.begin()),
                             std::make_move_iterator(rhs.end()));
    return temp;
}
...
MEOJSON_INLINE object object::operator|(object &&rhs) && {
    ...
    _object_data.insert(std::make_move_iterator(rhs.begin()),
                        std::make_move_iterator(rhs.end()));
    return std::move(*this);
}
...
template <typename MapType, typename EnableT> object::object(MapType map) {
    _object_data.insert(std::make_move_iterator(map.begin()),
...
MEOJSON_INLINE std::optional<value> open(std::ifstream &ifs,
...
MEOJSON_INLINE std::optional<value> open(const InputFilename &filepath,
                                         bool check_bom = false) {
...
#endif // INC_3D_RECONSTRUCTION_MEOJSON_HPP
